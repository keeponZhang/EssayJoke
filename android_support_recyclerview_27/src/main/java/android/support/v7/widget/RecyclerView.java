...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2 {
    ...
    private void initChildrenHelper() {
        mChildHelper = new ChildHelper(new ChildHelper.Callback() {
            ...
            public void addView(View child, int index) {
                ...
                Log.e("TAG", "RecyclerView 添加 addView 啦啦:");
    ...
    void consumePendingUpdateOperations() {
        Log.w("TAG", "RecyclerView RecyclerViewDataObserver consumePendingUpdateOperations:");
    ...
    boolean scrollByInternal(int x, int y, MotionEvent ev) {
        ...
        Log.d("TAG", "RecyclerView scrollByInternal 开始调用1 dispatchOnScrolled 参数y:" +y+ " consumedY="+consumedY+" unconsumedY="+unconsumedY);
        if (dispatchNestedScroll(consumedX, consumedY, unconsumedX, unconsumedY, mScrollOffset,
                TYPE_TOUCH)) {
            // Update the last touch co-ords, taking any scroll offset into account
            Log.e("TAG", "RecyclerView scrollByInternal mScrollOffset 偏移mScrollOffset[1] :"+mScrollOffset[1]+" 被消费:" );
        ...
        Log.d("TAG", "RecyclerView scrollByInternal consumedY:"+consumedY );
        if (consumedX != 0 || consumedY != 0) {
            Log.d("TAG", "RecyclerView scrollByInternal 开始调用2 dispatchOnScrolled:" );
    ...
    public boolean fling(int velocityX, int velocityY) {
        ...
        Log.w("TAG", "RecyclerView fling ####  调用 dispatchNestedPreFling:");
        boolean dispatchNestedPreFling = dispatchNestedPreFling(velocityX, velocityY);
        Log.w("TAG", "RecyclerView fling ####  调用 dispatchNestedPreFling 后return :"+dispatchNestedPreFling);
        if (!dispatchNestedPreFling) {
            final boolean canScroll = canScrollHorizontal || canScrollVertical;
            Log.d("TAG", "RecyclerView fling ####  调用  dispatchNestedFling :");
            ...
            if (canScroll) {
                ...
                Log.d("TAG", "RecyclerView fling ####  调用  startNestedScroll canScrollVertical :"+canScrollVertical);
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.d("TAG", "RecyclerView onInterceptTouchEvent ACTION_DOWN startNestedScroll:");
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                Log.d("TAG", "RecyclerView onTouchEvent ACTION_MOVE before dispatchNestedPreScroll dy:"+dy );
                if (dispatchNestedPreScroll(dx, dy, mScrollConsumed, mScrollOffset, TYPE_TOUCH)) {
                    dx -= mScrollConsumed[0];
                    dy -= mScrollConsumed[1];
                    Log.e("TAG", "RecyclerView onTouchEvent dispatchNestedPreScroll ACTION_MOVE mScrollOffset 偏移mScrollOffset[1] :"+mScrollOffset[1] );
                    vtev.offsetLocation(mScrollOffset[0], mScrollOffset[1]);
                    // Updated the nested offsets
                    mNestedOffsets[0] += mScrollOffset[0];
                    mNestedOffsets[1] += mScrollOffset[1];
                }
                Log.d("TAG", "RecyclerView onTouchEvent ACTION_MOVE after dispatchNestedPreScroll dy:" +dy);
...
//                    Log.w("TAG", "RecyclerView onTouchEvent Actino_move canScrollVertically:"+canScrollVertically);
                    ...
                    Log.d("TAG", "RecyclerView mScrollOffset onTouchEvent Actino_move canScrollVertically:"+canScrollVertically+  " startScroll ="+startScroll);
                }
                ...
                if (mScrollState == SCROLL_STATE_DRAGGING) {
                    ...
                    if (mGapWorker != null && (dx != 0 || dy != 0)) {
                        Log.w("TAG", "RcyLog RecyclerView onTouchEvent 要调用mGapWorker的run方法啦 dx:"+dx+" dy="+dy);
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        Log.w("TAG", "RecyclerView onMeasure------------------------------:");
        if (mLayout == null) {
...
// 可以发现这里其实是没有进行child的测量就直接return结束了onMeasure过程的，这也就解释了为什么我们没有设置LayoutManager会导致显示空白了。
    ...
    private void processAdapterUpdatesAndSetAnimationFlags() {
        Log.w("TAG", "RecyclerView  RecyclerViewDataObserver processAdapterUpdatesAndSetAnimationFlags:");
        if (mDataSetHasChangedAfterLayout) {
    ...
    void dispatchLayout() {
        ...
        if (mState.mLayoutStep == State.STEP_START) {
            ...
            // changed size.
            mLayout.setExactMeasureSpecsFrom(this);
            Log.e("TAG", "RecyclerView  dispatchLayoutStep2 dispatchLayout:");
            dispatchLayoutStep2();
        } else {
            // always make sure we sync them (to ensure mode is exact)
            mLayout.setExactMeasureSpecsFrom(this);
        }
        //如果上面情况都没有，那么onLayout的作用就仅仅是dispatchLayoutStep3()，而 dispatchLayoutStep3()方法的作用除了重置一些参数，
        // 外还和执行动画有关
        dispatchLayoutStep3();
    }
    ...
    private void dispatchLayoutStep3() {
        Log.e("TAG", "RecyclerView dispatchLayoutStep3:");
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        Log.e("TAG", "-------------------------------RecyclerView onLayout-------------------------------:");
        dispatchLayout();
        TraceCompat.endSection();
    ...
    private class RecyclerViewDataObserver extends AdapterDataObserver {
        ...
        public void onItemRangeChanged(int positionStart, int itemCount, Object payload) {
            assertNotInLayoutOrScroll(null);
            Log.e("TAG", "RecyclerViewDataObserver onItemRangeChanged positionStart:"+positionStart+" itemCount="+itemCount);
    ...
    public static class RecycledViewPool {
        ...
        //可以看到RecyclerdViewPool内部使用到了Google推荐的数据结构类型SparseArray，而SparseArray内部的key就是我们的ViewType，
        // 而value存放的是ArrayList<ViewHolder>,而默认的每个ArrayList<ViewHolder>的大小是5个
        ...
        //这里还有一个要注意的点就是getRecycledView这个方法可以看到拿到viewholder其实是通过remove拿到的，也就是通过remove拿到的
...
//    * 4.四级缓存：mRecyclerPool
    public final class Recycler {
        ...
        ViewHolder tryGetViewHolderForPositionByDeadline(int position,
            ...
            if (holder == null) {
                Log.e("TAG", "RcyLog Recycler tryGetViewHolderForPositionByDeadline 第一次尝试 mAttachedScrap.size():"+mAttachedScrap.size()+" position="+position);
            ...
            Log.e("TAG", "RcyLog Recycler tryGetViewHolderForPositionByDeadline 第一次尝试后 holder != null:"+(holder != null)+" position="+position);
            if (holder == null) {
                final int offsetPosition = mAdapterHelper.findPositionOffset(position);
                if (offsetPosition < 0 || offsetPosition >= mAdapter.getItemCount()) {
                    throw new IndexOutOfBoundsException("Inconsistency detected. Invalid item "
                ...
                //这里首先先看一个重点：mAdapter.getItemViewType(offsetPosition);熟悉的方法有木有，可以看到这里调用了我们平常使用RecyclerView进行多样式item的方法，
                // 也就是说前面对于一级缓存mAttachedScrap和mCacheViews是不区分type的，从现在开始的判断是区分type的缓存。这里对于我们研究多type类型的RecyclerView很有帮助。
                ...
                if (holder == null) { // fallback to pool
                    ...
                    // 第四次尝试(对应于Pool),从mRecyclerPool获取
            ...
            if (mState.isPreLayout() && holder.isBound()) {
                ...
                //如果FLAG是ViewHolder.FLAG_UPDATE | ViewHolder.FLAG_INVALID,则需要调bindViewHolder
        ...
        public void recycleView(View view) {
            ...
            Log.e("TAG", "RcyLog Recycler  recycleView 准备调用recycleViewHolderInternal，将ViewHolder加入CacheView和Pool中:");
            recycleViewHolderInternal(holder);
        }
         ...
         */
        //这个方法最终会将ViewHolder加入CacheView和Pool中
        ...
        void recycleViewHolderInternal(ViewHolder holder) {
            ...
            if (forceRecycle || holder.isRecyclable()) {
                if (mViewCacheMax > 0
                    ...
                    Log.w("TAG", "RcyLog Recycler recycleViewHolderInternal 准备放入 mCachedViews 啦啦 mCachedViews.size:"+mCachedViews.size()+" targetCacheIndex="+targetCacheIndex+"  holder.getLayoutPosition="+holder.getLayoutPosition());
                    //加入缓存
         ...
         */
        //1.函数1：
        // 可以知道，我们在自定义的LayoutManager中只需要告诉RecyclerView哪些HolderView已经不用了即可（使用removeAndRecycleView(view, recycler)函数）
        //mCachedViews是第一级缓存，它的size为2，只能保存两个HolderView。这里保存的始终是最新鲜被移除的HolderView，
        // 当mCachedViews满了以后，会利用先进先出原则，把老的HolderView存放在mRecyclerPool中。在mRecyclerPool中，
        // 它的默认size是5。这就是RecyclerView的回收原则
        //2.函数2：detachAndScrapAttachedViews
        //detachAndScrapAttachedViews(recycler);的作用就是把当前屏幕上所有的HolderView与屏幕分离，
        // 将它们从RecyclerView的布局中拿下来，然后存放在一个列表中，在重新布局时，像搭积木一样，
        // 把这些HolderView重新一个个放在新位置上去。将屏幕上的HolderView从RecyclerView的布局中拿下来后，
        // 存放的列表叫mAttachedScrap，它依然是一个List，就是用来保存从RecyclerView的布局中拿下来的HolderView列表

        // 所以，大家可以查看所有自定义的LayoutManager，detachAndScrapAttachedViews(recycler);只会被用在onLayoutChildren函数中。
        ...
        // 这些holderView并不参与回收复用。单纯只是为了先从RecyclerView中拿下来，再重新布局上去。对于新布局中没有用到的HolderView，
        // 会从mAttachedScrap移到mCachedViews中，让它参与复用。
        ...
        ViewHolder getScrapOrHiddenOrCachedHolderForPosition(int position, boolean dryRun) {
            ...
            Log.w("TAG", "RcyLog Recycler getScrapOrHiddenOrCachedHolderForPosition 从CacheView中拿 cacheSize:"+cacheSize+" position="+position);
            for (int i = 0; i < cacheSize; i++) {
                final ViewHolder holder = mCachedViews.get(i);
                Log.e("TAG", "Recycler getScrapOrHiddenOrCachedHolderForPosition holder.getLayoutPosition():"+holder.getLayoutPosition());
        ...
        //可以看到这里的判断其实和上面那一次差不多，需要注意的是多了对于id的判断和对于type的判断，也就是当我们将hasStableIds()设为true后
        // 需要重写holder.getItemId() 方法，来为每一个item设置一个单独的id
    ...
    public abstract static class LayoutManager {
        ...
        public void detachAndScrapAttachedViews(Recycler recycler) {
            ...
            final int childCount = getChildCount();
            for (int i = childCount - 1; i >= 0; i--) {
                //获取某个可见位置的View，需要非常注意的是，它的位置索引并不是Adapter中的位置索引，而是当前在屏幕上的位置的索引
        ...
        private void scrapOrRecycleView(Recycler recycler, int index, View view) {
            ...
            //这里就可以看到前面所说的Remove和Detach的区别，如果是remove，会执行recycleViewHolderInternal(viewHolder)方法，
            //而这个方法最终会将ViewHolder加入CacheView和Pool中，而当是Detach，会将View加入到ScrapViews中
            //，注意View和ViewHolder的区别，前面提到过，ScrapViews是对View的复用，而CacheView和Pool是对ViewHolder的复用。
    ...
    public abstract static class ViewHolder {
        ...
        //可以看到所有被put进入RecyclerPool中的ViewHolder都会被重置，这也就意味着RecyclerPool中的ViewHolder再被复用的时候是需要重新Bind的。
        // 这一点就可以区分和CacheViews中缓存的区别
        void resetInternal() {
            mFlags = 0;
    ...
    public boolean dispatchNestedScroll(int dxConsumed, int dyConsumed, int dxUnconsumed,
        ...
        boolean b = getScrollingChildHelper().dispatchNestedScroll(dxConsumed, dyConsumed,
                dxUnconsumed, dyUnconsumed, offsetInWindow, type);
        Log.d("TAG", "RecyclerView dispatchNestedScroll "+  " return="+b+"  dyConsumed:" +dyConsumed+" dyUnconsumed="+dyUnconsumed);
        return b;
    }

    @Override
    public boolean dispatchNestedPreScroll(int dx, int dy, int[] consumed, int[] offsetInWindow) {
        return dispatchNestedPreScroll(dy,dy,consumed,offsetInWindow,TYPE_TOUCH);
    }
    ...
    public boolean dispatchNestedPreScroll(int dx, int dy, int[] consumed, int[] offsetInWindow,
        ...
        boolean b = getScrollingChildHelper().dispatchNestedPreScroll(dx, dy, consumed, offsetInWindow,
                type);
        Log.d("TAG", "RecyclerView dispatchNestedPreScroll mScrollOffset return type:"+b+"  consumed[1]="+consumed[1] );
        return b ;
    }
