...
import com.android.coordinatorLayout.adapter.MyRecyclerviewAdapter;
...
//	请注意：上面提到的某个可滚动View，可以理解为某个ScrollView。怎么理解上面的话呢？就是说，当某个ScrollView发生滚动时，你可以定制你的“顶部栏”应该执行哪些动作（如跟着一起滚动、保持不动等等）。
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_coordinator_layout_appbarlayout_exituntil_collapsed_recyclerview);
	...
	private void loadData() {
		...
		for (int i = 0; i < 50; i++) {
			datas.add("this is test: " + i );
		}
		myRecyclerviewAdapter.updateDatas(datas);
	}
}
